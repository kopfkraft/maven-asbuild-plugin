    ...
    extends AbstractIdeaMojo
{
    ...
    private Map<String,String> libraryUUIDMap;
        ...
        throws MojoExecutionException
    {
        ...
        try
        {
            ...
            else if ( "swc".equals( executedProject.getPackaging()) || "swf".equals( executedProject.getPackaging() ))
            {
                addFlashModule( module );
            }
            ...
            // hack to link libs
            if(executedProject.getPackaging().equals("swf") || executedProject.getPackaging().equals("swc"))
            {
                Element newModuleRoot = findElement(module, "component", "NewModuleRootManager");
                Element dependencies = findElement(module, "configuration", executedProject.getArtifactId()).element("dependencies");
                linkFlashLibraries(newModuleRoot, dependencies);
            }
    ...
    private void linkFlashLibraries( Element moduleRoot, Element dependencies )
    {
        for  ( Iterator orderEntries = moduleRoot.elementIterator("orderEntry") ; orderEntries.hasNext();)
        {
            Element orderEntry = (Element)orderEntries.next();

            if(orderEntry.attribute("type").getValue().equals("module-library"))
            {
                Element lib = orderEntry.element("library");
                Element root = orderEntry.element("library").element("CLASSES").element("root");
                String url = root.attribute("url").getValue();

                if(url.toLowerCase().contains(".swc") && libraryUUIDMap != null)
                {
                    lib.addAttribute("type", "flex");
                    url = url.substring(0,url.length() - 2);
                    url = url.substring(url.lastIndexOf("/") + 1);

                    String lookup = libraryUUIDMap.get(url);

                    if(lookup != null)
                    {
                        log.info("found flex library named: " + url + " - " + lookup);
                        lib.addElement("properties").addAttribute("id",lookup);
                    }
                }
            }
        }
    ...
    private void addFlashModule( Element module )
    {

        module.addAttribute( "type", "Flex" );
        module.addAttribute( "version", "4" );

        //TODO read properties hack ... move it to pom.xml when there is time
        Properties properties = new Properties();
        try {
            properties.load(new FileReader(executedProject.getBasedir().getAbsolutePath() + "/src/conf/build.properties"));
        } catch (IOException e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
        }

        //TODO read compiler config
        //TODO check targetPlayer default value
        String ideaConfig = executedProject.getBasedir() + "/src/conf/compiler-idea.xml";
        ASUtils.copyAndReplace(executedProject.getBasedir() + "/src/conf/compiler-debug.xml", ideaConfig, properties);

        SAXReader reader = new SAXReader();
        Document ideaDoc = null;
        String targetPlayer = "11.1";

        try {
            ideaDoc = reader.read(ideaConfig);
            targetPlayer = ideaDoc.getRootElement().element("target-player").getTextTrim();

            log.info("found targetPlayer version: " + targetPlayer);

        } catch (Exception e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
            log.info("exception trying to read targetPlayer version");
        }

        //TODO take first FlexSDK out of idea jdk.table.xml
        //TODO fix idea version number in path
        File jdkFile = new File(System.getProperty("user.home") + "/.IntelliJIdea11/config/options/jdk.table.xml");
        Document jdkTable = null;
        String flexSdkId = "";
        try {

            log.info("looking for fdks in: " + jdkFile.getAbsolutePath());

            jdkTable = reader.read( jdkFile );
            List<Element> fdks = jdkTable.selectNodes( "//jdk" );

            log.info("found fdks: " + fdks.size());

            for (Element fdk: fdks)
            {
                String fdkType = fdk.element("type").attribute("value").getValue();
                String fdkName = fdk.element("name").attribute("value").getValue();
                log.info("found fdk: " + fdkType + " " + fdkName);

                if(fdkType.contains("Flex") || fdkType.contains("AIR"))
                {
                    //TODO find something that fits here?!?
                    flexSdkId = fdkName;
                    log.info("using " + fdkType + " with id " + fdkName);
                    break;
                }
            }
        } catch (Exception e) {
            e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
            log.info("couldnt find a proper flex sdk");
        }

        Element manager = module.addElement("component");
        manager.addAttribute("name","FlexBuildConfigurationManager");
        manager.addAttribute("active", executedProject.getArtifactId());
        
        Element conf = manager.addElement("configurations").addElement("configuration");
        conf.addAttribute("name", executedProject.getArtifactId());

        //TODO find somehting meaningful ... as classes could use flex as well ...
//        conf.addAttribute("pure-as", String.valueOf(properties.getProperty("maven.main", "Main.as").endsWith(".as")));
//        conf.addAttribute("pure-as", String.valueOf(properties.getProperty("maven.main", "Main.as").endsWith(".as")));


        if(executedProject.getPackaging().equals("swf"))
        {
            String mainClass = properties.getProperty("maven.main","Main.as").replaceAll("/",".");
            conf.addAttribute("main-class", mainClass.substring(0, mainClass.lastIndexOf(".")));
            conf.addAttribute("output-file", properties.getProperty("maven.nickname", executedProject.getArtifactId()) + ".swf");
        }
        else if(executedProject.getPackaging().equals("swc"))
        {
            conf.addAttribute("output-type", "Library");
            conf.addAttribute("output-file", properties.getProperty("maven.nickname", executedProject.getArtifactId()) + ".swc");
        }

        conf.addAttribute("output-folder", executedProject.getBuild().getOutputDirectory());

        Element dependencies = conf.addElement("dependencies");

        //TODO target-player from conf .. better pom.xml
        dependencies.addAttribute("target-player",targetPlayer);

        //TODO static linkage not taken from compiler.xml ?!?
        if(executedProject.getPackaging().equals("swf"))
            dependencies.addAttribute("framework-linkage","Merged");

        //TODO not yet sure where to get the sdk ... some conf files maybe
        Element sdk = dependencies.addElement("sdk");
        sdk.addAttribute("name", flexSdkId);

        Set<Artifact> depList = executedProject.getArtifacts();
        libraryUUIDMap = new HashMap<String, String>();

        for(Artifact dep : depList)
        {
            //TODO scope compile vs test
            if(dep.getType().equals("swc") )
            {
                Element entry = dependencies.addElement("entries").addElement("entry");

                if(isReactorProject(dep.getGroupId(), dep.getArtifactId()))
                {
                    // module dependency
                    entry.addAttribute("module-name", dep.getArtifactId());
                    entry.addAttribute("build-configuration-name", dep.getArtifactId());
                }
                else
                {
                    // library dependency
                    String uid = UUID.randomUUID().toString();
                    libraryUUIDMap.put(dep.getArtifactId() + "-" + dep.getVersion() + ".swc", uid);
                    log.info("storing flex library uid: " + dep.getArtifactId() + "-" + dep.getVersion() + ".swc - " + uid);
                    entry.addAttribute("library-id", uid);
                }

                if(executedProject.getPackaging().equals("swc"))
                {
                    entry.addElement("dependency").addAttribute("linkage", "External");
                }
                else
                {
                    entry.addElement("dependency").addAttribute("linkage", "Merged");
                }
            }
        }

        Element compiler = conf.addElement("compiler-options");
        Element coption = compiler.addElement("option");
        coption.addAttribute("name","additionalConfigFilePath");
        coption.addAttribute("value","$MODULE_DIR$/src/conf/compiler-idea.xml");

        conf.addElement("packaging-air-desktop");
        conf.addElement("packaging-android");
        conf.addElement("packaging-ios");
